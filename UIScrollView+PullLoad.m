...
//                NSLog(@"tableview:%@, downview:%@", NSStringFromCGSize(((UITableView *)downView.superview).contentSize), NSStringFromCGRect(downView.frame));
            }
        }
...
- (void)stopLoadWithState:(LoadState)state {
    if (state == PullDownLoadState && self.interceptor.upView.state == PullStateLoading) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            UIEdgeInsets insets = self.contentInset;
            insets = UIEdgeInsetsMake(insets.top, 0, 10, 0);
            [self setContentInset:insets];
        }];
